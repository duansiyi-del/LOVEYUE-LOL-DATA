...
1. **SGP (腾讯服务端战绩)** —— 脚本用客户端给的 token 在【本机】直接拉, 再把对局
   发给网站入库. 深度不受客户端缓存限制, 一次能要 100 场.
   ⚠ SGP 只认"token 是谁的就查谁": 拿本机账号的 token 去查队友会 401, 所以这条路
   只能拿到【本机登录这个账号】的完整历史. 想每个人的历史都全, 就各自在自己机器上
   跑一次. (客户端本地接口反而没有这个限制, 查谁都行, 只是每人只给 20 场.)
   ⚠ 也不要让网站去拉: 网站跑在境外, 实测只有客户端所在这台机器发的请求能通.
   (之前一直以为这条路不通: 当时主机名写成了大写 GZ100-sgp..., 网关对 Host 精确
   匹配, 所以任何路径都返回 400. 改成小写 gz100-sgp... 就通了, 2026-09-24 实测.)
2. **客户端本地接口** —— SGP 不通时的退路. ⚠ 国服客户端【每人只给最近 20 场, 而且
   忽略翻页参数】, 2026-09-24 八个成员挨个试过全是 20 场, 换写法、一次要 100 条都
   一样. 所以这条路只能覆盖最近这点对局.
